...
public class ReviewControllerIT extends BaseIT{
	...
	void should_return_all_reviews() throws Exception {
	    ...
	    api.perform(builder)
	       ...
	       .andExpect(jsonPath("$[0].timeStamp").value(org.hamcrest.Matchers.containsString("2025-12-07T12:05:52")))
	       
	       .andExpect(jsonPath("$[1].userEmail", is("bob@example.com")))   
	       .andExpect(jsonPath("$[1].comment", is("content2")))           
	       .andExpect(jsonPath("$[1].timeStamp").value(org.hamcrest.Matchers.containsString("2025-12-07T12:08:10")));
	}
}